import requests
import json
import pandas as pd
import gspread
from gspread_dataframe import get_as_dataframe, set_with_dataframe

#dataframe creation options
pd.options.mode.copy_on_write = True
pd.set_option('display.max_colwidth',None)

#API info, google drive json files, sheets setup.
API_TOKEN = "70f0dd699921dba321fe3f10cbe0d112"
url = "https://api.start.gg/gql/alpha"
credentials = 'C:/Users/andhy/Desktop/code/scripts/StartGG/credentials-sheets.json'
authorized_user = 'C:/Users/andhy/Desktop/code/scripts/StartGG/authorized_user.json'
gc = gspread.oauth()
SheetsID = "1KMcrNygaSwDygcz8pCLL0VeqDECsHxVXgGhaoF0E7gM"
sh = gc.open_by_key(SheetsID)
worksheet_name = 'PlacementImport'
worksheet = sh.worksheet(worksheet_name)
dfSheets = get_as_dataframe(worksheet)

#Query made to the StartGG API.
body = """
query standings($tourneySlug : String!, $page: Int!, $perPage: Int!, $eventId: ID){
  tournament(slug: $tourneySlug){
    participants(query:{
      page: $page
      perPage: $perPage
      filter: {eventIds: [$eventId]} 
    })
    {
      nodes {
        gamerTag
        user{
          discriminator
        }
        entrants{
          team {
            name
          }
          standing{
          	placement 
          }
        }
      }
    }
  }
},
"""
#Variables header to be passed to the query.
variables = {
  "tourneySlug" : input("Enter Tourney Slug:"),
  "eventId": input("Enter EventID: "),
  "page" : 1,
  "perPage" : 500,
  "SplatoonID" : 36202,
  "limit" : 10
}

#Headers passed to the query, along with the query and variables itself.
payload = {"query": body,
            "variables": variables}
headers = {"Authorization": "Bearer 70f0dd699921dba321fe3f10cbe0d112",
"Content-Type": "application/json"}

# Send the POST request
response = requests.post(url, json=payload, headers=headers)

if response.status_code == 200:
    data = response.json()
else:
    print(f"Request failed with status code: {response.status_code}")
    print(response.text)    

df = pd.json_normalize(data['data']['tournament']['participants']['nodes'])
print(df)
# Creating new columns and extracting data from nested dictionaries for those columns.
#Append teamname values to df
df['teamname'] = None
for x in range(len(df)):
    try:
        df.iloc[x,3]= str((df['entrants'][x][0]['team']['name']))
    except:
        df.iloc[x,3]= "N/A"
    
#append placement values to df.
df['placement'] = None
for x in range(len(df)):
    try:
        df.iloc[x,4]= (df['entrants'][x][0]['standing']['placement'])
    except:
        df.iloc[x,4]= "N/A"

# Create an event ID via user input for the new Dataframe.
new_col_values = input("Enter the Event ID: ")
df.insert(loc=0, column='EventID', value=(new_col_values))

#Remove redundant nested dictionaries & Entrants column.
del df['entrants']

#Apppend Queried Dataframe into Google Sheets Dataframe via Concat function.
#bigdf = [dfSheets, df]
#dfSheets = pd.concat(bigdf)
df_list = df.values.tolist()
#Upload new merged Dataframe to Google Sheets.
row = 1
col = 1
worksheet.append_rows(df_list)

print(df)
print(dfSheets)